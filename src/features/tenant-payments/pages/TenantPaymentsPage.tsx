      ...
      {rejectId && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <form onSubmit={handleRejectSubmit} className="bg-white rounded-2xl max-w-sm w-full p-5 border border-slate-100 shadow-2xl">
            ...
            <textarea required value={reasonInput} onChange={e => setReasonInput(e.target.value)} placeholder={en ? 'E.g., Transfer amount does not match booking invoice.' : 'Contoh: Jumlah transfer kurang dari invoice booking.'} className="w-full border border-slate-200 p-2.5 rounded-xl text-xs bg-slate-50 min-h-22.5 focus:outline-hidden font-semibold text-slate-700 mb-4" />
